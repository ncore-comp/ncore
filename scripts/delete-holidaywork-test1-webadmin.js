...
function isTargetHolidayWork(row = {}) {
  const userId = text(row.userId);
  const reportCategory = text(row.reportCategory);
  const type = text(row.type);
  if (!TARGET_USER_IDS.has(userId)) return false;
  return reportCategory === "holiday_work" || type === "특근";
}

async function main() {
  const db = initAdmin();
  const snap = await db.collection("requests").get();
  const targetDocs = snap.docs.filter((doc) => isTargetHolidayWork(doc.data()));

  if (!targetDocs.length) {
    console.log(JSON.stringify({
      result: "skip",
      deleted: 0,
      message: "no target holiday work rows found"
    }, null, 2));
    return;
  }
