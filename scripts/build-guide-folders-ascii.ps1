...
$github = Join-Path (Join-Path $root "legacy") "package_github_2026-03-26"
$firestore = Join-Path (Join-Path $root "legacy") "package_firestore_ops_2026-03-26"
...
foreach ($f in $githubFiles) {
    ...
    if (Test-Path $src) { Copy-Item $src $github -Force }
}

Get-ChildItem $root -File -Filter "*.md" | ForEach-Object {
    if ($_.DirectoryName -eq $root) { Copy-Item $_.FullName $github -Force }
}
...
foreach ($f in $firestoreFiles) {
    $src = Join-Path $root $f
    if (Test-Path $src) { Copy-Item $src $firestore -Force }
}
...
foreach ($f in $firestoreScripts) {
    ...
    if (Test-Path $src) { Copy-Item $src $scriptDst -Force }
}
...
foreach ($f in $backupFiles) {
    ...
    if (Test-Path $src) { Copy-Item $src $backupDst -Force }
}
