...
const SCRIPT_REQUEST_TIMEOUT_MS = 15000;
    ...
    const security = {
        ...
        normalizeSpecialLeaveExpiryBasis: (value) => {
            const safe = String(value ?? '').trim().toLowerCase();
            return safe === 'event_date' ? 'event_date' : '';
        },
        normalizeSpecialLeaveExpiryDirection: (value) => {
            const safe = String(value ?? '').trim().toLowerCase();
        ...
        formatAccessLogTimestamp: (value) => {
            ...
            if (raw === 'TIMEOFF_OUT_INVALID_RANGE') return '외출 시간을 다시 확인해주세요.';
            if (raw === 'TIMEOFF_OUT_BEFORE_SHIFT_START') return '선택한 직원의 출근시간 이전에는 외출을 등록할 수 없습니다.';
            if (raw === 'TIMEOFF_OUT_AT_OR_AFTER_SHIFT_END') return '선택한 직원의 퇴근시간을 포함하거나 지난 외출은 등록할 수 없습니다.';
            if (raw === 'SPECIAL_LEAVE_EXPIRY_RULE_REQUIRED') return '영업일 기준 특별휴가는 소멸 방향과 기한 일수를 입력해야 합니다.';
            if (raw === 'SPECIAL_LEAVE_EVENT_DATE_REQUIRED') return '특별휴가 발생일을 입력해주세요.';
            if (raw === 'SPECIAL_LEAVE_GRANT_ID_REQUIRED') return '특별휴가 부여 건 정보가 없습니다.';
            if (raw === 'SPECIAL_LEAVE_GRANT_ID_DUPLICATED') return '특별휴가 부여 건 ID가 중복되었습니다.';
            if (raw === 'SPECIAL_LEAVE_GRANT_REQUIRED') return '특별휴가 부여 건을 선택해주세요.';
            if (raw === 'SPECIAL_LEAVE_GRANT_NOT_FOUND') return '선택한 특별휴가 부여 건을 찾을 수 없습니다.';
            if (raw === 'SPECIAL_LEAVE_WINDOW_INVALID') return '특별휴가 사용 가능 기간을 계산할 수 없습니다.';
            if (raw === 'SPECIAL_LEAVE_OUTSIDE_WINDOW') return '선택한 특별휴가는 사용 가능 기간 안에서만 신청할 수 있습니다.';
            if (raw === 'SPECIAL_LEAVE_GRANT_INSUFFICIENT') return '선택한 특별휴가 부여 건의 잔여가 부족합니다.';
            return raw;
        },
        isValidEmail: (value) => {
        ...
        sanitizePermissions: (permissions = {}, role = '', dept = '') => {
            ...
            const clean = {
                ...
                boardRead: permissions.boardRead !== undefined ? !!permissions.boardRead : true,
                boardWrite: permissions.boardWrite !== undefined ? !!permissions.boardWrite : false,
            ...
            if (role === 'master') {
                ...
                clean.boardRead = true;
                clean.boardWrite = true;
            ...
            if (role === 'ceo') {
                clean.boardRead = true;
                clean.boardWrite = false;
                clean.workReportViewManual = true;
                clean.workReportViewParts = true;
                clean.workReportViewAll = true;
            }
            if (role === 'team_leader' || role === 'part_leader' || role === 'employee') {
                clean.boardRead = permissions.boardRead !== undefined ? !!permissions.boardRead : true;
                clean.boardWrite = permissions.boardWrite !== undefined ? !!permissions.boardWrite : false;
            }
        ...
        sanitizeRequest: (req = {}) => {
            ...
            return {
                ...
                specialLeaveGrantId: security.cleanInlineValue(req.specialLeaveGrantId || ''),
                ...
                settlementStatus: security.cleanInlineValue(req.settlementStatus || ''),
                ...
                settlementApprovedAt: security.normalizeTimestamp(req.settlementApprovedAt || ''),
                settlementApprovedBy: security.cleanInlineValue(req.settlementApprovedBy || ''),
                settlementApprovedByName: security.cleanText(req.settlementApprovedByName || ''),
                settlementRejectedAt: security.normalizeTimestamp(req.settlementRejectedAt || ''),
                settlementRejectedBy: security.cleanInlineValue(req.settlementRejectedBy || ''),
                settlementRejectedByName: security.cleanText(req.settlementRejectedByName || ''),
                settlementRejectReason: security.cleanText(req.settlementRejectReason || '')
            };
        },
        ...
        sanitizeSpecialLeaveType: (item = {}) => ({
            ...
            expiryBasis: security.normalizeSpecialLeaveExpiryBasis(item.expiryBasis || (security.normalizeSpecialLeaveDayCountMode(item.dayCountMode) === 'business_days' ? 'event_date' : '')),
            expiryDirection: security.normalizeSpecialLeaveExpiryDirection(item.expiryDirection),
            expiryDays: Math.max(0, security.normalizeNumber(item.expiryDays, 0))
        }),
        sanitizeUserSpecialLeave: (item = {}) => ({
            userId: security.cleanInlineValue(item.userId),
            grantId: security.cleanInlineValue(item.grantId || `${item.userId || ''}__${item.typeKey || ''}`),
            typeKey: security.normalizeSpecialLeaveTypeKey(item.typeKey),
            grantedHours: Math.max(0, security.normalizeNumber(item.grantedHours ?? item.totalHours, 0)),
            totalHours: Math.max(0, security.normalizeNumber(item.grantedHours ?? item.totalHours, 0)),
            usedHours: Math.max(0, security.normalizeNumber(item.usedHours, 0)),
            pendingHours: Math.max(0, security.normalizeNumber(item.pendingHours, 0)),
            availableHours: Math.max(0, security.normalizeNumber(item.availableHours, 0)),
            eventDate: security.normalizeDate(item.eventDate, ''),
            usableFromDate: security.normalizeDate(item.usableFromDate, ''),
            usableToDate: security.normalizeDate(item.usableToDate, ''),
            grantStatus: ['active', 'consumed', 'expired', 'inactive'].includes(String(item.grantStatus ?? '').trim().toLowerCase())
                ? String(item.grantStatus ?? '').trim().toLowerCase()
                : 'active',
        ...
        upsertReq: async (reqData, options = {}) => {
            ...
            try {
                ...
                if (json.result !== 'success') {
                    ...
                    if (String(json.message || '') === 'TIMEOFF_OUT_BEFORE_SHIFT_START') {
                        throw new Error(`TIMEOFF_OUT_BEFORE_SHIFT_START:${Number(json.shiftStartHour || 0)}:${Number(json.shiftEndHour || 0)}`);
                    }
                    if (String(json.message || '') === 'TIMEOFF_OUT_AT_OR_AFTER_SHIFT_END') {
                        throw new Error(`TIMEOFF_OUT_AT_OR_AFTER_SHIFT_END:${Number(json.shiftStartHour || 0)}:${Number(json.shiftEndHour || 0)}`);
                    }
                    if (String(json.message || '') === 'TIMEOFF_OUT_INVALID_RANGE') {
                        throw new Error(`TIMEOFF_OUT_INVALID_RANGE:${Number(json.shiftStartHour || 0)}:${Number(json.shiftEndHour || 0)}`);
                    }
                ...
                } else if (msg.startsWith('TIMEOFF_OUT_BEFORE_SHIFT_START:')) {
                    const parts = msg.split(':');
                    alert(`선택한 직원의 출근시간(${String(Number(parts[1] || 0)).padStart(2, '0')}:00) 이전에는 외출을 등록할 수 없습니다.`);
                } else if (msg.startsWith('TIMEOFF_OUT_AT_OR_AFTER_SHIFT_END:')) {
                    const parts = msg.split(':');
                    alert(`선택한 직원의 퇴근시간(${String(Number(parts[2] || 0)).padStart(2, '0')}:00)을 포함하거나 지난 외출은 등록할 수 없습니다.`);
                } else if (msg.startsWith('TIMEOFF_OUT_INVALID_RANGE:')) {
                    alert('외출 시간을 다시 확인해주세요.');
                } else {
                    alert('\uC624\uB958: ' + msg);
                }
                return false;
