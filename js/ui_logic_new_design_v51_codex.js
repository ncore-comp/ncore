    const app = {
        ...
        renderRecentRequestRow: (r) => {
            ...
            else if (app.isHalfDayType(displayType)) badgeClass = 'bg-orange-100 text-orange-800';
            else if (displayType === '시간차(퇴근)') badgeClass = 'bg-yellow-200 text-yellow-900';
            else if (displayType === '시간차(외출)') badgeClass = 'bg-stone-200 text-stone-800';
        ...
        renderRecentRequestList: (title, requests, emptyText, options = {}) => {
            ...
            const paginationWrapClass = String(options.paginationWrapClass || 'flex justify-center items-center mt-4 space-x-2');
        ...
        buildMonthlyCalendarData: (requests, viewer, year = app.viewYear, month = app.viewMonth) => {
            ...
            Object.keys(eventMap).forEach((dateStr) => {
                eventMap[dateStr].sort((a, b) => {
                    const pA = typePriority[security.normalizeType(a.type)] || 99;
                    const pB = typePriority[security.normalizeType(b.type)] || 99;
        ...
        tryLogin: async () => {
            ...
            } catch(e) { console.error(e); alert("서버 통신 오류"); } finally { app.hideLoadingOverlay(); }
        },
        ...
        editReq: (id) => {
            ...
            document.getElementById('req-type').value = normalizedType === '반차' ? '반차(오후)' : normalizedType;
        ...
        renderManagerDashboard: () => {
            ...
            const myReqsSection = isEffectiveCEO ? '' : app.renderRecentRequestList('내 최근 신청 내역', myReqs, '신청 내역이 없습니다.', { paginationWrapClass: 'flex justify-center items-center -mt-6 mb-8 space-x-2' });
            ...
            const managerQuickAction = isEffectiveCEO ? '' : `<div class="mb-4 flex justify-end"><button onclick="app.openRequestModal()" class="bg-indigo-600 text-white px-6 py-2.5 rounded-lg font-bold shadow-md shadow-indigo-200"><i class="fa-solid fa-plus mr-2"></i> 연차 신청</button></div>`;
