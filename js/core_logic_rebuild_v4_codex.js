const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzvz1rjpk_l87O4dvizz0F8N_WT-SUHPzF-o_hxzoacknONrgm6AYQRd_b74Y1opGFZ/exec";
const CLIENT_IP_URL = "https://api64.ipify.org?format=json";
    ...
    const security = {
        ...
        normalizeBoardCategory: (value) => ['怨듭?', '?쇰컲', '?낅Т怨듭쑀', '吏덈Ц'].includes(String(value ?? '').trim()) ? String(value ?? '').trim() : '?쇰컲',
        ...
        sanitizePermissions: (permissions = {}, role = '', dept = '') => {
            ...
            // 留덉뒪????怨꾩젙? ??븷紐낆씠 ?꾨땲?? ??λ맂 permissions 媛믪쑝濡쒕쭔 沅뚰븳???먮떒?쒕떎.
        ...
        sanitizeRequest: (req = {}) => {
            ...
            return {
                ...
                type: security.normalizeType(String(req.type ?? '?곗감')),
                startDate,
                endDate,
    ...
    const db = {
        ...
        loadWithScope: async (scope = 'all', options = {}) => {
            try {
                ...
                console.error(e);
                return { ok: false, scopedLoadSupported: false, scope: String(scope || 'all') };
            }
        },
        load: async () => db.loadWithScope('all', { users: true, requests: true, boardPosts: true, holidays: true }),
        loadBoot: async () => db.loadWithScope('boot', { users: true, requests: false, boardPosts: false, holidays: true }),
        loadDeferred: async () => db.loadWithScope('rest', { users: false, requests: true, boardPosts: true, holidays: false }),
            ...
            try {
                ...
                const json = await db.postJsonWithRetry(
                    { action: 'upsert_request', data: cleanReq, actor: db.getActor() },
        ...
        upsertUser: async (userData) => {
            document.getElementById('loading-text').innerText = '\uC9C1\uC6D0 \uC815\uBCF4 \uC800\uC7A5 \uC911...';
            document.getElementById('loading-overlay').style.display = 'flex';
            try {
    ...
    function makeDurationOptions(maxHour = 10) {
        ...
        for (let i = 1; i <= maxHour; i++) {
            options += `<option value="${i}">${i}?쒓컙</option>`;
        }
        return options;
    }
