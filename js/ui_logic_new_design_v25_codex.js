    const app = {
        ...
        getApproverUser: (requester = app.currentUser) => {
            ...
            if (requester.role === 'employee') {
                const deptManager = candidates.find(u => u.role === 'manager' && u.dept === requester.dept);
        ...
        updateUser: async (id) => {
            try {
                ...
                if (!nameInput || !deptInput || !rankInput || !daysInput) {
        ...
        renderEmployeeDashboard: () => {
            ...
            document.getElementById('app-container').innerHTML = `<div class="w-full max-w-7xl mx-auto px-4 fade-in pt-8 pb-12"><div class="bg-white p-6 rounded-2xl shadow-sm border mb-8 flex justify-between items-center"><div><h2 class="text-2xl font-bold">내 연차 현황</h2><p class="text-sm text-gray-500 mt-1">${u.dept} | 총 ${app.fmtTime(u.totalHours)}</p></div><div class="text-right"><div class="text-4xl font-bold text-indigo-600">${app.fmtTime(u.totalHours-u.usedHours)} <span class="text-lg text-gray-400 font-normal">/ ${app.fmtTime(u.totalHours)}</span></div>${u.pendingHours > 0 ? `<div class="text-sm font-bold text-orange-500 mt-1">(승인 대기: ${app.fmtTime(u.pendingHours)})</div>` : ''}</div></div><div class="mb-4 flex justify-end"><button onclick="document.getElementById('req-modal').classList.remove('hidden'); app.initModal();" class="bg-indigo-600 text-white px-6 py-2.5 rounded-lg font-bold shadow-md shadow-indigo-200"><i class="fa-solid fa-plus mr-2"></i> 연차 신청</button></div><div class="mb-10">${app.renderCal(appData.requests, u)}</div><div class="flex justify-between items-center mb-4"><h3 class="text-lg font-bold">최근 신청 내역</h3></div><div class="bg-white rounded-xl shadow-sm border overflow-hidden">${totalItems===0?'<div class="p-8 text-center text-gray-400">내역 없음</div>':pagedReqs.map(r=> {
                const displayType = security.normalizeType(r.type);
                let badgeClass = '';
        ...
        renderManagerDashboard: () => {
            ...
            let dashboardTitle = (u.role === 'manager') ? '팀장 대시보드' : '직원 대시보드';
        ...
        renderMasterPermissionPage: () => {
            ...
            const rows = users.map(user => {
                ...
                const isManagerRow = user.role === 'manager';
                const isPartLeaderRow = ['최진', '안여란', '김은지', '김도형'].includes(String(user.name || '').trim());
                ...
                const roleBadges = [
                    ...
                    isManagerRow ? '<span class="ml-1 px-2 py-0.5 rounded-full text-[11px] bg-blue-100 text-blue-800 font-bold">팀장</span>' : '',
                    isPartLeaderRow ? '<span class="ml-1 px-2 py-0.5 rounded-full text-[11px] bg-rose-100 text-rose-800 font-bold">파트장</span>' : ''
                ].join('');
            ...
            const permissionPanel = `<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                <div class="p-4 border-b border-gray-200">
                    ...
                    <div class="mt-2 text-xs text-gray-500 flex flex-wrap gap-2 items-center">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 font-bold">팀장 행 강조</span>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 font-bold">대표 행 강조</span>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-rose-100 text-rose-800 font-bold">파트장 행 강조</span>
                    </div>
                </div>
                <div class="overflow-x-auto">
                    ${groupButtons}
                    <table id="master-perm-table" class="w-full text-sm table-fixed ${isCalendarOpen ? 'min-w-[1120px]' : 'min-w-[1040px]'}">
                        ${permissionColGroup}
