...
Object.assign(app, {
            ...
            buildSpecialLeaveTypeDraft: (item = {}, index = 0) => {
                ...
                return {
                    ...
                    expiryBasis: security.normalizeSpecialLeaveExpiryBasis(item.expiryBasis || 'event_date'),
                    expiryDirection: security.normalizeSpecialLeaveExpiryDirection(item.expiryDirection),
                    expiryDays: Math.max(0, Number(item.expiryDays || 0))
                };
            },
            ...
            syncSpecialLeaveTypeExpiryInputState: (container) => {
                const root = container && container.nodeType === 1 ? container : null;
                if (!root) return;
                const dayCountModeInput = root.querySelector('[data-special-type-day-count-mode]');
                const expiryDirectionInput = root.querySelector('[data-special-type-expiry-direction]');
                const expiryDaysInput = root.querySelector('[data-special-type-expiry-days]');
                const expiryHint = root.querySelector('[data-special-type-expiry-hint]');
                const isBusinessDays = dayCountModeInput && dayCountModeInput.value === 'business_days';
                if (expiryDirectionInput) {
                    expiryDirectionInput.disabled = !isBusinessDays;
                    expiryDirectionInput.classList.toggle('opacity-50', !isBusinessDays);
                    if (!isBusinessDays) expiryDirectionInput.value = '';
                }
                if (expiryDaysInput) {
                    expiryDaysInput.disabled = !isBusinessDays;
                    expiryDaysInput.classList.toggle('opacity-50', !isBusinessDays);
                    if (!isBusinessDays) expiryDaysInput.value = '';
                }
                if (expiryHint) {
                    expiryHint.innerText = isBusinessDays
                        ? '발생일 기준 · 발생 당일 미포함 · 전후 10일은 앞 10일 + 뒤 10일입니다.'
                        : '달력일 기준은 소멸기한 규칙을 사용하지 않습니다.';
                }
            },
        renderSpecialLeaveTypeSettingRow: (type, index = 0) => {
                ...
                const isBusinessDays = draft.dayCountMode === 'business_days';
                return `<div class="rounded-xl border px-4 py-3 ${colorClass}" data-special-type-key="${draft.typeKey}" data-special-type-color="${draft.color}">
                    <div class="grid grid-cols-1 xl:grid-cols-[minmax(240px,1.4fr)_120px_160px_170px] gap-3 items-end">
                        ...
                        <div>
                            ...
                            <select data-special-type-day-count-mode class="w-full border bg-white p-2.5 rounded" onchange="app.syncSpecialLeaveTypeExpiryInputState(this.closest('[data-special-type-key]'))">
                    ...
                    <div class="mt-3 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-[150px_120px_120px_minmax(170px,1fr)_auto_auto] gap-3 items-end">
                        <div>
                            <label class="block text-xs font-bold text-gray-500 mb-1">발생일 기준</label>
                            <input type="text" value="발생일 기준" class="w-full border bg-gray-100 p-2.5 rounded text-gray-500" readonly>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-500 mb-1">소멸 방향</label>
                            <select data-special-type-expiry-direction class="w-full border bg-white p-2.5 rounded ${isBusinessDays ? '' : 'opacity-50'}" ${isBusinessDays ? '' : 'disabled'}>
                                <option value="" ${!draft.expiryDirection ? 'selected' : ''}>선택</option>
                                <option value="before" ${draft.expiryDirection === 'before' ? 'selected' : ''}>전</option>
                                <option value="after" ${draft.expiryDirection === 'after' ? 'selected' : ''}>후</option>
                                <option value="both" ${draft.expiryDirection === 'both' ? 'selected' : ''}>전후</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-500 mb-1">기한 일수</label>
                            <input type="number" min="0" step="1" data-special-type-expiry-days value="${draft.expiryDays > 0 ? draft.expiryDays : ''}" class="w-full border bg-white p-2.5 rounded ${isBusinessDays ? '' : 'opacity-50'}" ${isBusinessDays ? '' : 'disabled'}>
                        </div>
                        <label class="inline-flex min-h-[46px] items-center gap-2 rounded-lg border bg-white px-3 py-2.5">
                            <input type="checkbox" data-special-type-allow-holiday class="accent-indigo-600" ${draft.allowHolidayRequest ? 'checked' : ''}>
                            <span class="text-sm font-bold leading-5 text-gray-700">주말/공휴일 신청 허용</span>
                        </label>
                        <label class="inline-flex min-h-[46px] items-center justify-start xl:justify-end cursor-pointer rounded-lg border border-transparent px-1 py-2">
                        ...
                        <button type="button" onclick="app.deleteSpecialLeaveTypeSettingRow('${draft.typeKey}')" class="min-h-[46px] w-full xl:w-auto xl:justify-self-end px-3 py-2.5 rounded-lg border border-red-200 text-red-600 text-sm font-bold hover:bg-red-50">삭제</button>
                    </div>
                    <div data-special-type-expiry-hint class="mt-3 text-xs text-gray-500">${isBusinessDays ? '발생일 기준 · 발생 당일 미포함 · 전후 10일은 앞 10일 + 뒤 10일입니다.' : '달력일 기준은 소멸기한 규칙을 사용하지 않습니다.'}</div>
                </div>`;
            },
            ...
            collectSpecialLeaveTypesFromSettings: () => {
                ...
                return rows.map((row, index) => {
                    ...
                    const expiryDirectionInput = row.querySelector('[data-special-type-expiry-direction]');
                    const expiryDaysInput = row.querySelector('[data-special-type-expiry-days]');
                    ...
                    const dayCountMode = dayCountModeInput ? dayCountModeInput.value : 'business_days';
                    const expiryDirection = dayCountMode === 'business_days' ? (expiryDirectionInput ? expiryDirectionInput.value : '') : '';
                    const expiryDays = dayCountMode === 'business_days' ? Number(expiryDaysInput ? expiryDaysInput.value : 0) : 0;
                    return app.buildSpecialLeaveTypeDraft({
                        ...
                        dayCountMode,
                        expiryBasis: dayCountMode === 'business_days' ? 'event_date' : '',
                        expiryDirection,
                        expiryDays: Number.isFinite(expiryDays) && expiryDays > 0 ? expiryDays : 0
                    }, index);
                }).filter(Boolean);
            },
            saveMasterSettings: async () => {
                ...
                const workReportToggle = document.getElementById('global-workreport-toggle');
                ...
                const workReportEnabled = workReportToggle ? workReportToggle.checked : app.isAnyWorkReportFeatureEnabled();
                ...
                const invalidExpiryType = specialLeaveTypes.find((type) =>
                    security.normalizeSpecialLeaveDayCountMode(type.dayCountMode) === 'business_days' &&
                    (
                        !security.normalizeSpecialLeaveExpiryDirection(type.expiryDirection) ||
                        Number(type.expiryDays || 0) <= 0
                    )
                );
                if (invalidExpiryType) {
                    return alert(`${invalidExpiryType.label || '특별휴가'} 소멸 방향과 기한 일수를 입력해주세요.`);
                }
                ...
                try {
                    const savedMaster = await db.upsertUser({
                        ...
                        featureOvertime: workReportEnabled,
                        featureHolidayWork: workReportEnabled,
                    ...
                    try {
                        ...
                        await app.logUserAction('SettingSave', `memberCard:${memberCardEnabled}|homepage:${homepageEnabled}|board:${boardEnabled}|workReport:${workReportEnabled}|special:${specialSummary}`);
                    } catch (e) {
            ...
            renderMasterPermissionPage: () => {
                ...
                const workReportEnabled = app.isAnyWorkReportFeatureEnabled();
    ...
    <button type="button" onclick="app.addSpecialLeaveTypeFromInput()" class="px-4 py-2.5 rounded-lg text-sm font-bold border border-indigo-200 text-indigo-600 hover:bg-indigo-50 shrink-0">추가</button>
                ...
                const workReportSettingsPanel = `<div class="bg-white border border-gray-200 rounded-xl shadow-sm p-4 mb-4">
                    ...
                    <div class="mt-3 pt-3 border-t border-gray-100 flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <p class="text-sm font-bold text-gray-700">잔업/특근 신청 기능</p>
                            <p class="text-xs text-gray-500">직원이 개인 장부에 잔업과 특근을 기록하고 승인 요청할 수 있습니다.</p>
                        </div>
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" id="global-workreport-toggle" class="sr-only peer" ${workReportEnabled ? 'checked' : ''}>
                            <div class="relative w-10 h-5 bg-gray-200 rounded-full peer peer-checked:bg-indigo-600 peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:w-4 after:h-4 after:bg-white after:border after:border-gray-300 after:rounded-full after:transition-all"></div>
                            <span class="ml-2 text-sm font-bold ${workReportEnabled ? 'text-indigo-700' : 'text-gray-500'}">${workReportEnabled ? '켜짐' : '꺼짐'}</span>
                        </label>
                    </div>
                ...
                const activeGroupSubHeader = isCalendarOpen
                    ...
                    : (isApproveOpen
                        ...
                        : (isMemberStatusOpen
                            ...
                            : (isBoardOpen
                                ? `<th class="p-3 bg-cyan-50 border-b border-gray-200 text-center border-l border-cyan-100">읽기</th>
                                    <th class="p-3 bg-cyan-50 border-b border-gray-200 text-center border-r border-cyan-100">쓰기</th>`
                ...
                const activeGroupColSpan = isCalendarOpen ? 3 : (isDetailOpen ? 6 : (isSituationOpen ? 2 : (isBoardOpen ? 2 : 3)));
                ...
                const permissionPanel = `<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div class="overflow-x-auto">
                        ${groupButtons}
                        <table id="master-perm-table" class="w-full text-sm table-fixed ${isCalendarOpen ? 'min-w-[980px]' : (isDetailOpen ? 'min-w-[1190px]' : (isSituationOpen ? 'min-w-[930px]' : (isWorkReportOpen ? 'min-w-[980px]' : (isBoardOpen ? 'min-w-[930px]' : 'min-w-[1140px]'))))}">
                            ${permissionColGroup}
                    ...
                    : (activeTab === 'settings'
                        ...
                        : (activeTab === 'mail'
                            ...
                            : (activeTab === 'workreport'
                                ? workReportSettingsPanel
                                : `<div id="admin-ops-panel-root">${app.renderAdminOpsPanel()}</div>`)));
