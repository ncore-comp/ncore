    ...
    renderCal: (requests, viewer) => {
        ...
        for(let d=1; d<=lastDate; d++) {
            ...
            const eventBody = app.isMobileViewport
                ? `<div class="flex h-[calc(100%-6px)] flex-col ${applyShading ? 'opacity-60 grayscale' : ''}"><div>${app.renderMobileCalendarDaySummary(evts)}</div><div class="mt-auto">${specialLeaveDecorationHtml}</div></div>`
                : `<div class="px-1 flex h-[calc(100%-6px)] flex-col ${applyShading ? 'opacity-60 grayscale' : ''}"><div class="min-h-0">${evtHtml ? `<div class="grid grid-cols-2 gap-1 overflow-y-auto max-h-[82px] custom-scrollbar">${evtHtml}</div>` : ''}</div><div class="mt-auto">${specialLeaveDecorationHtml}</div></div>`;
