    ...
    renderCal: (requests, viewer) => {
        ...
        for(let d=1; d<=lastDate; d++) {
            ...
            const eventBody = app.isMobileViewport
                ? `<div class="${applyShading ? 'opacity-60 grayscale' : ''}">${app.renderMobileCalendarDaySummary(evts)}${app.renderCalendarSpecialLeaveEventChips(specialGrantEvents, { mobile: true })}${app.renderCalendarSpecialLeaveBars(specialGrantBars)}</div>`
                : `<div class="px-1 ${applyShading ? 'opacity-60 grayscale' : ''}">${app.renderCalendarSpecialLeaveEventChips(specialGrantEvents)}<div class="grid grid-cols-2 gap-1 overflow-y-auto max-h-[82px] custom-scrollbar">${evtHtml}</div>${app.renderCalendarSpecialLeaveBars(specialGrantBars)}</div>`;
