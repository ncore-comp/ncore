...
Object.assign(app, {
    ...
    getCalendarSpecialLeaveDecorations: (viewer, year = app.viewYear, month = app.viewMonth) => {
        ...
        const overlapPalette = ['emerald', 'sky', 'violet', 'amber', 'rose', 'indigo'];
        const displayColorByGrant = new Map();
        if (entries.length > 1) {
            entries.forEach((entry, index) => {
                displayColorByGrant.set(entry.grantId, overlapPalette[index % overlapPalette.length]);
            });
        }
        ...
        entries.forEach((entry) => {
            ...
            const displayColor = displayColorByGrant.get(entry.grantId) || entry.color;
    ...
    renderCal: (requests, viewer) => {
        ...
        for(let d=1; d<=lastDate; d++) {
            ...
            const specialLeaveFooterHtml = hasSpecialLeaveDecoration
                ? `<div class="mt-auto px-1 pb-1 pt-2 ${applyShading ? 'opacity-60 grayscale' : ''}">${specialLeaveDecorationHtml}</div>`
                : '';
            const eventBody = app.isMobileViewport
                ? `<div class="flex-1 min-h-0 ${applyShading ? 'opacity-60 grayscale' : ''}">${app.renderMobileCalendarDaySummary(evts)}</div>`
                : `<div class="flex-1 min-h-0 px-1 ${applyShading ? 'opacity-60 grayscale' : ''}">${evtHtml ? `<div class="grid grid-cols-2 gap-1 overflow-y-auto max-h-[82px] custom-scrollbar">${evtHtml}</div>` : ''}</div>`;
            html += `<div class="calendar-cell hover:bg-gray-50 ${applyShading ? 'bg-gray-50' : ''} ${todayClass} ${selectedClass}" data-cal-date="${dateStr}" ${clickAttr} ${dblClickAttr} ${hoverAttrs}><div class="h-full flex flex-col"><div class="flex justify-between items-start mb-1 ${applyShading ? 'opacity-40' : ''}"><span class="text-sm font-bold ml-1 mt-1 ${isToday?'today-circle':(isRed?'holiday-text':(isSat?'sat-text':'text-gray-700'))}">${d}</span>${holName ? `<span class="text-[10px] text-white bg-red-400 px-1 rounded truncate max-w-[60px]">${holName}</span>` : ''}</div>${eventBody}${specialLeaveFooterHtml}</div></div>`;
        }
        html += `</div>`;
