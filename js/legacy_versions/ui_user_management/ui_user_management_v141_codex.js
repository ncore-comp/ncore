...
Object.assign(app, {
    ...
    buildUserSpecialLeaveGrantDraft: (userId, typeKey, item = {}) => {
        const safeUserId = security.cleanInlineValue(userId);
        const safeTypeKey = security.normalizeSpecialLeaveTypeKey(typeKey || item.typeKey);
        return security.sanitizeUserSpecialLeave({
            userId: safeUserId,
            grantId: item.grantId || `grant_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
            typeKey: safeTypeKey,
            eventDate: item.eventDate || '',
            grantedHours: Number((item.grantedHours ?? item.totalHours) || 0),
            usedHours: Number(item.usedHours || 0),
            note: item.note || '',
            usableFromDate: item.usableFromDate || '',
            usableToDate: item.usableToDate || '',
            grantStatus: item.grantStatus || 'active'
        });
    },
    getSpecialLeaveGrantLinkedRequestCount: (userId, typeKey, grantId) => {
        const safeUserId = security.cleanInlineValue(userId);
        ...
        return (Array.isArray(appData.requests) ? appData.requests : []).filter((req) => {
            ...
            if (security.cleanInlineValue(req.specialLeaveGrantId || '') === safeGrantId) return true;
            return !req.specialLeaveGrantId && security.normalizeSpecialLeaveTypeKey(req.specialLeaveTypeKey) === safeTypeKey;
        }).length;
    },
    renderUserSpecialLeaveGrantRow: (userId, typeMeta, grant = {}) => {
        const draft = app.buildUserSpecialLeaveGrantDraft(userId, typeMeta.typeKey, grant);
        const linkedCount = app.getSpecialLeaveGrantLinkedRequestCount(userId, typeMeta.typeKey, draft.grantId);
        const locked = linkedCount > 0;
        const window = app.getSpecialLeaveUsableWindow(draft, typeMeta);
        const windowText = draft.eventDate
            ? (window.usableFromDate && window.usableToDate ? `${window.usableFromDate} ~ ${window.usableToDate}` : '소멸기한 규칙 없음')
            : '발생일 입력 시 사용 가능 기간이 계산됩니다.';
        return `<div class="rounded-lg border border-gray-200 bg-white p-3" data-special-leave-grant-row data-type-key="${draft.typeKey}" data-grant-id="${draft.grantId}">
            <div class="flex items-center justify-between gap-3 mb-3">
                <div>
                    <p class="text-sm font-bold text-gray-800">${typeMeta.label}</p>
                    <p class="text-[11px] text-gray-500">${locked ? '요청 연결됨' : '수정 가능'} · ${linkedCount}건 연결</p>
                </div>
                <button type="button" onclick="app.removeUserSpecialLeaveGrantRow('${draft.grantId}')" class="px-3 py-2 rounded-lg border border-red-200 text-red-600 text-xs font-bold hover:bg-red-50 ${locked ? 'opacity-40 cursor-not-allowed' : ''}" ${locked ? 'disabled' : ''}>삭제</button>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                    <label class="block text-xs font-bold text-gray-500 mb-1">발생일</label>
                    <input type="date" data-grant-event-date value="${draft.eventDate || ''}" class="w-full border bg-white p-2.5 rounded ${locked ? 'opacity-60' : ''}" ${locked ? 'disabled' : ''}>
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-500 mb-1">부여 일수</label>
                    <input type="number" min="0" step="1" data-grant-days value="${draft.grantedHours > 0 ? app.formatDayCount(draft.grantedHours) : ''}" class="w-full border bg-white p-2.5 rounded ${locked ? 'opacity-60' : ''}" ${locked ? 'disabled' : ''}>
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-500 mb-1">수동 사용 일수</label>
                    <input type="number" min="0" step="1" data-grant-used-days value="${draft.usedHours > 0 ? app.formatDayCount(draft.usedHours) : ''}" class="w-full border bg-white p-2.5 rounded ${locked ? 'opacity-60' : ''}" ${locked ? 'disabled' : ''}>
                </div>
            </div>
            <div class="mt-3 rounded-lg bg-gray-50 border border-gray-200 px-3 py-2 text-xs text-gray-600">
                <div class="font-bold text-gray-700 mb-1">사용 가능 기간</div>
                <div>${windowText}</div>
            </div>
        </div>`;
    },
    addUserSpecialLeaveGrantRow: (typeKey) => {
        const safeTypeKey = security.normalizeSpecialLeaveTypeKey(typeKey);
        const safeUserId = security.cleanInlineValue(document.getElementById('edit-user-id')?.value || '');
        const typeMeta = app.getSpecialLeaveTypeMeta(safeTypeKey);
        const wrap = document.getElementById(`special-leave-grants-${safeTypeKey}`);
        if (!safeUserId || !typeMeta || !wrap) return;
        const draft = app.buildUserSpecialLeaveGrantDraft(safeUserId, safeTypeKey, { grantedHours: Number(typeMeta.grantHours || 0) });
        wrap.insertAdjacentHTML('beforeend', app.renderUserSpecialLeaveGrantRow(safeUserId, typeMeta, draft));
    },
    removeUserSpecialLeaveGrantRow: (grantId) => {
        const safeGrantId = security.cleanInlineValue(grantId);
        const row = document.querySelector(`[data-special-leave-grant-row][data-grant-id="${safeGrantId}"]`);
        if (row) row.remove();
    },
    renderUserSpecialLeaveGrantEditor: (userId) => {
        const types = app.getSortedSpecialLeaveTypes({ enabledOnly: false });
        if (!types.length) {
        ...
        const grantEntries = app.getUserSpecialLeaveGrantEntries(userId, { enabledOnly: false, includeZero: true });
        return `<div class="rounded-xl border border-gray-200 bg-gray-50/70 p-4">
            <div class="mb-3">
                <p class="text-sm font-bold text-gray-700">특별휴가</p>
                <p class="text-xs text-gray-500">경조사 한 건마다 grant를 추가합니다. 발생일은 필수이며, 요청 연결 이력이 있으면 자유 수정/삭제할 수 없습니다.</p>
            </div>
            <div class="space-y-4">
                ${types.map((type) => {
                    const rows = grantEntries.filter((grant) => grant.typeKey === type.typeKey);
                    return `<div class="rounded-xl border bg-white p-4 ${app.getSpecialLeaveColorClasses(type.color)}">
                        <div class="flex items-center justify-between gap-3 mb-3">
                            <div>
                                <p class="text-sm font-bold">${type.label}</p>
                                <p class="text-xs text-gray-500">발생일 기준 / ${type.dayCountMode === 'business_days' ? '영업일 기준' : '달력일 기준'}</p>
                            </div>
                            <button type="button" onclick="app.addUserSpecialLeaveGrantRow('${type.typeKey}')" class="px-3 py-2 rounded-lg border border-indigo-200 text-indigo-600 text-xs font-bold hover:bg-indigo-50">부여건 추가</button>
                        </div>
                        <div id="special-leave-grants-${type.typeKey}" class="space-y-3">
                            ${rows.length ? rows.map((grant) => app.renderUserSpecialLeaveGrantRow(userId, type, grant)).join('') : `<div class="rounded-lg border border-dashed border-gray-200 bg-gray-50 px-3 py-4 text-xs text-gray-400">아직 부여된 ${type.label}이 없습니다.</div>`}
                        </div>
                    </div>`;
                }).join('')}
            </div>
    ...
    collectUserSpecialLeaveGrantPayload: (userId) => {
        const safeUserId = security.cleanInlineValue(userId);
        const rows = [...document.querySelectorAll('[data-special-leave-grant-row]')];
        return rows.map((row) => {
            const typeKey = security.normalizeSpecialLeaveTypeKey(row.getAttribute('data-type-key'));
            const grantId = security.cleanInlineValue(row.getAttribute('data-grant-id'));
            const eventDate = security.normalizeDate(row.querySelector('[data-grant-event-date]')?.value, '');
            const grantedDays = Number(row.querySelector('[data-grant-days]')?.value || 0);
            const usedDays = Number(row.querySelector('[data-grant-used-days]')?.value || 0);
            if (!typeKey) return null;
            if (!eventDate) throw new Error('특별휴가 발생일을 입력해주세요.');
            if (!Number.isFinite(grantedDays) || grantedDays <= 0) throw new Error('특별휴가 부여 일수를 입력해주세요.');
            const linkedCount = app.getSpecialLeaveGrantLinkedRequestCount(safeUserId, typeKey, grantId);
            const typeMeta = app.getSpecialLeaveTypeMeta(typeKey);
            const window = app.getSpecialLeaveUsableWindow({ typeKey, eventDate }, typeMeta);
            return {
                grantId,
                userId: safeUserId,
                typeKey,
                eventDate,
                grantedHours: grantedDays * 8,
                totalHours: grantedDays * 8,
                usedHours: Number.isFinite(usedDays) && usedDays > 0 ? usedDays * 8 : 0,
                usableFromDate: window.usableFromDate,
                usableToDate: window.usableToDate,
                grantStatus: linkedCount > 0 ? 'inactive' : 'active'
            };
        }).filter(Boolean);
    },
    ...
    openUserEditModal: (id) => {
        ...
        if (specialLeaveWrap) specialLeaveWrap.innerHTML = app.renderUserSpecialLeaveGrantEditor(safeId);
    ...
    updateUser: async (id) => {
        try {
            ...
            const specialLeavePayload = app.collectUserSpecialLeaveGrantPayload(safeId);/*
    ...
    renderUserManagement: () => {
        ...
        const list = app.getManageScopeUsers()
            ...
            .sort((a, b) => {
                ...
                const empA = String(a?.employeeNo || '').trim();
                const empB = String(b?.employeeNo || '').trim();
                const byEmployeeNo = empA.localeCompare(empB, 'ko');
                if (byEmployeeNo !== 0) return byEmployeeNo;
                return String(a?.id || '').localeCompare(String(b?.id || ''), 'ko');
            });
        document.getElementById('app-container').innerHTML = `<div class="w-full max-w-[1720px] mx-auto px-4 xl:px-6 pt-8 fade-in pb-12">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <table class="w-full text-left table-fixed">
                    <colgroup>
                        ...
                        ${app.currentUser.role==='master' ? '<col style="width:70px;">' : ''}
                        <col style="width:120px;">
                    </colgroup>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ${list.map(u => {
                            ...
                            return `<tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="p-4 text-sm text-gray-400 text-xs break-all">${u.id}</td>
                                <td class="p-4 text-sm text-gray-600 break-all">${u.employeeNo || '-'}</td>
                                <td class="p-4 text-sm text-gray-600 break-all">${u.phone || '-'}</td>
                                <td class="p-4 text-sm text-gray-600 break-all">${u.email || '-'}</td>
