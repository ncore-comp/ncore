        ...
        const dailyAbsentItems = absentTypeGroups
            .map((typeGroup) => {
                ...
                const itemMarkup = isExpanded
                    ? `<div class="mt-3 space-y-2">${typeGroup.items.map((item) => {
                        const timeDetail = item.timeRange
                            ? `<div class="mt-1 text-xs text-gray-500">${item.timeRange}</div>`
                            : '';
                        return `<div class="rounded-xl border border-gray-100 bg-white px-3 py-3"><div class="font-bold text-gray-800">${item.userName}</div><div class="mt-1 text-xs text-gray-500">${item.dept}</div>${timeDetail}</div>`;
                    }).join('')}</div>`
                    : '';
