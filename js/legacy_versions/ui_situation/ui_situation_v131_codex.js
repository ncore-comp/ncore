...
Object.assign(app, {
    ...
    renderSituationBoard: () => {
        ...
        for (let d = 1; d <= lastDate; d++) {
            ...
            cellsHtml += `<div class="situation-board-cell ${isToday ? 'is-today' : ''} ${isSelected ? 'is-selected' : ''}" data-situation-date="${dateStr}" style="min-height:${scale.cellMinHeight}px" onclick="app.openSituationBoardSeatMap('${dateStr}')"><div class="flex h-full flex-col"><div class="flex items-start justify-between gap-2"><div class="flex items-start gap-2 min-w-0"><span class="${scale.cellDateClass} font-black ${dateColorClass}">${d}</span>${holName ? `<span class="mt-1 text-[10px] text-white bg-red-400 px-1.5 py-0.5 rounded truncate max-w-[78px]">${holName}</span>` : ''}</div></div><div class="mt-2 grid grid-cols-2 gap-1.5">${chips.map((chip) => `<div class="px-2 py-1 rounded-full font-bold leading-tight ${chip.cls} ${scale.chipClass}">${chip.label}</div>`).join('') || `<div class="text-gray-300 ${scale.chipClass} font-medium col-span-2">-</div>`}</div>${summary.totalUsers > 0 ? `<div class="mt-auto pt-3 text-right text-[11px] font-bold text-slate-500">연차 인원 : ${summary.totalUsers}명</div>` : ''}</div></div>`;
        }
