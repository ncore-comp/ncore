...
Object.assign(app, {
    ...
    getSituationBoardScaleConfig: () => {
        ...
        if (scale === 'xlarge') {
            return { cardNumberClass: 'text-4xl', cardLabelClass: 'text-sm', monthTitleClass: 'text-4xl', cellDateClass: 'text-2xl', totalClass: 'text-xl', chipClass: 'text-[15px]', countLabelClass: 'text-[14px]', holidayLabelClass: 'text-xs', detailTitleClass: 'text-2xl', detailTextClass: 'text-base', cellMinHeight: 142, chipLimit: 4 };
        }
        if (scale === 'normal') {
            return { cardNumberClass: 'text-3xl', cardLabelClass: 'text-xs', monthTitleClass: 'text-3xl', cellDateClass: 'text-lg', totalClass: 'text-lg', chipClass: 'text-[12px]', countLabelClass: 'text-[12px]', holidayLabelClass: 'text-[10px]', detailTitleClass: 'text-xl', detailTextClass: 'text-sm', cellMinHeight: 104, chipLimit: 4 };
        }
        return { cardNumberClass: 'text-4xl', cardLabelClass: 'text-sm', monthTitleClass: 'text-4xl', cellDateClass: 'text-xl', totalClass: 'text-xl', chipClass: 'text-[13px]', countLabelClass: 'text-[13px]', holidayLabelClass: 'text-[11px]', detailTitleClass: 'text-2xl', detailTextClass: 'text-sm', cellMinHeight: 124, chipLimit: 4 };
    },
    ...
    renderSituationBoard: () => {
        ...
        for (let d = 1; d <= lastDate; d++) {
            ...
            cellsHtml += `<div class="situation-board-cell ${isToday ? 'is-today' : ''} ${isSelected ? 'is-selected' : ''}" data-situation-date="${dateStr}" style="min-height:${scale.cellMinHeight}px" onclick="app.openSituationBoardSeatMap('${dateStr}')"><div class="flex h-full flex-col"><div class="flex items-start justify-between gap-2"><div class="flex items-start gap-2 min-w-0"><span class="${scale.cellDateClass} font-black ${dateColorClass}">${d}</span>${holName ? `<span class="mt-1 ${scale.holidayLabelClass} text-white bg-red-400 px-1.5 py-0.5 rounded truncate max-w-[78px]">${holName}</span>` : ''}</div></div><div class="mt-2 grid grid-cols-2 gap-1.5">${chips.map((chip) => `<div class="px-2.5 py-1.5 rounded-full font-bold leading-tight ${chip.cls} ${scale.chipClass}">${chip.label}</div>`).join('') || `<div class="text-gray-300 ${scale.chipClass} font-medium col-span-2">-</div>`}</div>${summary.totalUsers > 0 ? `<div class="mt-auto pt-3 text-right ${scale.countLabelClass} font-bold text-slate-500">연차 인원 : ${summary.totalUsers}명</div>` : ''}</div></div>`;
        }
