...
Object.assign(app, {
        ...
        getUserSpecialLeaveGrantEntries: (userId, options = {}) => {
            ...
            return grants
                .map((grant) => {
                    ...
                    const grantStatus = grant.grantStatus === 'inactive' || missingEventDate
                        ? 'inactive'
                        : (availableHours <= 0 ? 'consumed' : (isExpired ? 'expired' : 'active'));
                    return {
                        ...meta,
                        ...grant,
        ...
        getUserSpecialLeaveEntries: (userId, options = {}) => {
            ...
            grantEntries.forEach((grant) => {
                if (grant.grantStatus === 'expired' || grant.grantStatus === 'inactive') return;
            ...
            return [...grouped.values()].filter((item) => includeZero || item.granted);
        },
        ...
        getRequestTypeOptionsHtml: (selectedValue = '연차') => {
            ...
            const specialOptions = requestUser
                ? app.getAvailableSpecialLeaveGrantEntries(requestUser.id, { includeGrantId: selection.specialLeaveGrantId }).map((entry) => ({
