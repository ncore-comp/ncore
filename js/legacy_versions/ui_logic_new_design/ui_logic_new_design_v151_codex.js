    Object.assign(app, {
        ...
        specialLeaveLoginPopupShown: false,
        ...
        showSpecialLeaveLoginPopup: () => {
            if (!app.currentUser || app.specialLeaveLoginPopupShown) return;
            const summary = app.getSpecialLeaveLoginPopupSummary(app.currentUser.id, { today: moment().format('YYYY-MM-DD') });
            const expired = Array.isArray(summary.expired) ? summary.expired : [];
            const warning = Array.isArray(summary.warning) ? summary.warning : [];
            if (!expired.length && !warning.length) return;
            const lines = ['[특별휴가 안내]'];
            lines.push(`- 소멸 ${expired.length}건 / 임박 ${warning.length}건`);
            if (expired.length) {
                lines.push('');
                lines.push('[소멸]');
                expired.slice(0, 5).forEach((item) => lines.push(`- ${item.label} / 발생일 ${item.eventDate || '-'} / 남은 ${app.fmtTime(item.availableHours)}`));
            }
            if (warning.length) {
                lines.push('');
                lines.push('[임박]');
                warning.slice(0, 5).forEach((item) => lines.push(`- ${item.label} / 발생일 ${item.eventDate || '-'} / 남은 ${app.fmtTime(item.availableHours)}`));
            }
            lines.push('');
            lines.push('달력에서 확인하세요.');
            app.specialLeaveLoginPopupShown = true;
            alert(lines.join('\n'));
        },
        ...
        getAdminOpsTargetUsers: () => {
            ...
            return appData.users.filter((u) => !!u);
        },
        ...
        refreshAdminOpsPanelOnly: () => {
            const root = document.getElementById('admin-ops-panel-root');
            if (
                app.currentView === 'master-permissions' &&
                app.masterPermissionTab === 'ops' &&
                root
            ) {
                root.innerHTML = app.renderAdminOpsPanel();
                return;
            }
            app.renderMasterPermissionPage();
        },
        ...
        loadAdminOpsAccessLogs: async () => {
            ...
            if (ok) app.refreshAdminOpsPanelOnly();
        },
        ...
        restoreSession: async () => {
            ...
            try {
                ...
                app.recalcUsedHours();
                app.renderNav();
                app.specialLeaveLoginPopupShown = false;
                app.refreshDashboard();
                app.showSpecialLeaveLoginPopup();
                return true;
            } catch (e) {
        ...
        renderNav: () => {
            ...
            const boardEnabled = app.canReadBoard();
        ...
        canEditBoardPost: (post) => {
            ...
            return app.canWriteBoard() && String(post.authorId) === String(app.currentUser.id);
        },
        ...
        openBoardWriteForm: (postId = '') => {
            ...
            if (!target && !app.canWriteBoard()) {
                alert('이 글은 작성 권한이 없습니다.');
                return;
            }
        ...
        saveBoardPost: async () => {
            if (!app.currentUser) return;
            if (!app.canWriteBoard()) return alert('이 글은 작성 권한이 없습니다.');
            const titleEl = document.getElementById('board-title');
            const categoryEl = document.getElementById('board-category');
            const contentEl = document.getElementById('board-content');
            const noticeEl = document.getElementById('board-notice');
            ...
            <div class="w-full max-w-7xl mx-auto px-4 fade-in pt-8 pb-12">
                <div class="bg-white p-6 rounded-2xl shadow-sm border mb-6">
                    <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
                        ...
                        <div class="flex items-center gap-2">
                            ${app.canWriteBoard() ? '<button onclick="app.openBoardWriteForm()" class="bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold">글쓰기</button>' : ''}
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl transform scale-100">
                    ...
                    <div class="space-y-4 mb-6">
                        ...
                        <div>
                            ...
                            <select id="req-type" class="w-full bg-gray-50 border p-2.5 rounded-lg" onchange="toggleInputs(); app.checkHoliday(document.getElementById('req-start-date'))">${app.getRequestTypeOptionsHtml('연차')}</select>
                        </div>
                        <div id="div-special-mode" class="hidden">
                            <label class="block text-xs font-bold text-gray-500 mb-1">사용 방식</label>
                            <select id="req-special-mode" class="w-full bg-gray-50 border p-2.5 rounded-lg" onchange="toggleInputs(); app.checkHoliday(document.getElementById('req-start-date'))">${app.getSpecialModeOptionsHtml('연차')}</select>
                        </div>
                        ...
                        <div id="div-out" class="hidden bg-emerald-50 p-3 rounded-lg border border-emerald-100">
                            <div class="flex gap-2 mb-2">
                                <div class="w-1/2">
                                    ...
                                    <select id="req-duration-out" class="w-full border p-1 rounded mt-1" onchange="app.handleTimeOffOutDurationChange()">${outDurationOpts}</select>
                                </div>
                                <div class="w-1/2">
                                    <label class="text-xs font-bold text-emerald-700">시작 시간</label>
                                    <select id="req-start-time-out" class="w-full border p-1 rounded mt-1" onchange="updateOutPreview()"></select>
                                </div>
                            </div>
    ...
    app.checkHoliday = (el) => {
        const dateStr = security.normalizeDate(el?.value, '');
        ...
        const currentTypeValue = document.getElementById('req-type')?.value || '연차';
        const specialSelectionRequested = String(currentTypeValue).startsWith('specialgrant:') || String(currentTypeValue).startsWith('special:');
        const dateRange = app.getCurrentRequestDateRange();
        const resolvedTypeValue = app.refreshRequestTypeOptions(currentTypeValue, {
            dateStr: dateRange.startDate || dateStr,
            endDateStr: dateRange.endDate || dateStr
        });
        app.syncRequestTypeDefaults();
        if (specialSelectionRequested && resolvedTypeValue !== currentTypeValue) {
            statusEl.innerText = '⚠️ 선택한 특별휴가는 현재 날짜에 신청할 수 없어 종류 목록에서 제외되었습니다.';
        ...
        if (selection.isSpecial && requestUser) {
            selectedGrant = app.getUserSpecialLeaveGrantEntries(requestUser.id, { enabledOnly: false, includeZero: true })
                .find((item) => item.grantId === selection.specialLeaveGrantId) || null;
            if (!selectedGrant && selection.specialLeaveTypeKey) {
                const sameTypeEntries = app.getUserSpecialLeaveGrantEntries(requestUser.id, { enabledOnly: false, includeZero: true })
                    .filter((item) => item.typeKey === selection.specialLeaveTypeKey);
                if (sameTypeEntries.length === 1) selectedGrant = sameTypeEntries[0];
            }
        }
        if (selectedGrant && selectedGrant.usableFromDate && selectedGrant.usableToDate) {
            const endDate = dateRange.endDate || dateStr;
            if (dateStr < selectedGrant.usableFromDate || endDate > selectedGrant.usableToDate) {
                statusEl.innerText = `⚠️ 선택한 특별휴가는 사용 가능 기간만 신청할 수 있습니다. (사용 가능: ${selectedGrant.usableFromDate} ~ ${selectedGrant.usableToDate})`;
                statusEl.style.color = "#ef4444";
                toggleInputs();
                return;
            }
    ...
    function calcOutEndTime(startHour, durationHour) {
        ...
        return app.calcTimeOffOutEndHour(startHour, durationHour, shift.endHour);
    }
    ...
    function updateOutPreview() {
        ...
        const startDateEl = document.getElementById('req-start-date');
        const dateStr = security.normalizeDate(startDateEl?.value, moment().format('YYYY-MM-DD'));
        const validation = app.getTimeOffOutValidation(requestUser, dateStr, s, d);
        if (!validation.valid) {
            if (validation.reason === 'before_shift_start') {
                preview.innerText = `* 선택한 직원의 출근시간은 ${formatHour(validation.shiftStartHour)}입니다. 그 이전에는 외출을 등록할 수 없습니다.`;
            } else if (validation.reason === 'at_or_after_shift_end') {
                preview.innerText = `* 선택한 직원의 퇴근시간(${formatHour(validation.shiftEndHour)})을 포함하거나 지난 외출은 등록할 수 없습니다.`;
            } else {
                preview.innerText = '* 외출 시간을 다시 확인해주세요.';
            }
            preview.style.color = '#dc2626';
            return;
        }
        preview.innerText = `자동 종료 시간: ${formatHour(validation.endHour)} (${formatHour(s)} ~ ${formatHour(validation.endHour)})`;
        preview.style.color = '#047857';
    }

        app.handleTimeOffOutDurationChange = () => {
            app.syncTimeOffOutOptions();
            updateOutPreview();
        };
