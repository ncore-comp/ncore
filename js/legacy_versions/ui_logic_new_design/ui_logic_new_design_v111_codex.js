    const app = {
        ...
        renderManagerDashboard: () => {
            ...
            const situationButton = app.canAccessSituationBoard() ? `<button onclick="app.openSituationBoard()" class="bg-slate-800 text-white px-4 py-2.5 rounded-lg font-bold hover:bg-slate-900">전체 상황판</button>` : '';
            document.getElementById('app-container').innerHTML = `<div class="w-full max-w-7xl mx-auto px-4 fade-in pt-8 pb-12"><div class="flex justify-between items-center mb-6"><h2 class="text-2xl font-bold">${dashboardTitle}</h2>${situationButton}</div>${myStatusCard}${managerQuickAction}<div class="mb-8">${app.renderCal(appData.requests, app.currentUser)}</div>${approvalSection}${myReqsSection}${memberStatusHtml}</div>${app.getModal()}`;
            app.initModal();
        },
