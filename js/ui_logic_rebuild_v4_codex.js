    const app = {
        ...
        tryLogin: async () => {
            ...
            try {
                ...
                if (json.result === 'success') {
                    ...
                    if (bootResult.ok && bootResult.scopedLoadSupported) {
                        db.loadDeferred()
                            .then(() => {
                                if (!app.currentUser) return;
                                app.recalcUsedHours();
        ...
        refreshDashboard: () => {
            ...
            app.ensureCalendarMode();
            try {
                if(app.currentUser.role === 'master' || app.currentUser.role === 'ceo' || app.hasApprovalPermission()) app.renderManagerDashboard();
        ...
        processReq: async (id, action) => {
            ...
            const savedReq = await db.upsertReq(req);
            if (!savedReq) return;
            ...
            modal.innerHTML = `
                <div class="bg-white rounded-2xl p-6 w-full max-w-2xl shadow-2xl">
                    ...
                    <div id="edit-user-pw-wrap" class="mb-4">
                        ...
                        <input id="edit-user-pw" type="password" placeholder="새 비밀번호(8자 이상)" class="w-full border bg-gray-50 p-2.5 rounded">
                    </div>
                    <div class="flex justify-end gap-2">
        ...
        renderManagerDashboard: () => {
            ...
            else if (isCEO) dashboardTitle = '대표 대시보드';
        ...
        saveMasterPermissions: async () => {
            ...
            const boardEnabled = document.getElementById('global-board-toggle')?.checked || false;
            ...
            if (freshMe) app.currentUser = { ...app.currentUser, permissions: freshMe.permissions, featureMemberCard: freshMe.featureMemberCard, featureBoard: freshMe.featureBoard };
            app.renderNav();
            await app.logUserAction('PermissionSave', `memberCard:${memberCardEnabled}|board:${boardEnabled}`);
            alert('권한 저장 완료');
