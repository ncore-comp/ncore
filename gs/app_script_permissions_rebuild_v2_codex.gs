  ...
  try {
    ...
    if (action === "load") {
      ...
      if (userSheet.getLastRow() > 1) {
        const data = userSheet.getRange(2, 1, userSheet.getLastRow() - 1, 22).getValues();
        users = data.map((r) => {
...
function doPost(e) {
  try {
    ...
    if (action === "login") {
      ...
      const row = data.slice(1).find((r) => String(r[0]).trim() === inputId && String(r[1]).trim() === inputPw);
    ...
    try {
      ...
      if (action === "save_users") {
        ...
        if (userSheet.getLastRow() > 1) {
          const oldData = userSheet.getRange(2, 1, userSheet.getLastRow() - 1, 2).getValues();
          oldData.forEach((r) => {
        ...
        if (userSheet.getLastRow() > 1) {
          userSheet.getRange(2, 1, userSheet.getLastRow() - 1, 22).clearContent();
        }
        ...
        if (newUsers.length > 0) {
          userSheet.getRange(2, 1, newUsers.length, 22).setValues(newUsers);
        }
        recordLog(
          ss,
          String(actor.id || ""),
      ...
      if (action === "delete_board_post") {
        ...
        for (let i = 1; i < data.length; i++) {
          if (String(data[i][0]) === targetId) {
            boardSheet.getRange(i + 1, 9).setValue("deleted");
            boardSheet.getRange(i + 1, 12).setValue(now);
