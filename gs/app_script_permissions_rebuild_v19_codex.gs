...
const REQUEST_HEADERS = [
  ...
  "reason", "status", "timestamp", "specialLeaveTypeKey", "specialLeaveTypeLabel"
];
    ...
    try {
      if (action === "upsert_request") {
        ...
        const rowData = [
          ...
          r.reason, r.status, r.timestamp, String(r.specialLeaveTypeKey || ""), String(r.specialLeaveTypeLabel || "")
        ];
...
function ensureRequestHeaders(reqSheet) {
  ...
  const oldHeaders = REQUEST_HEADERS.slice(0, 13);
  ...
  if (matchesOld) {
    reqSheet.getRange(1, 1, 1, REQUEST_HEADERS.length).setValues([[...oldHeaders, REQUEST_HEADERS[13], REQUEST_HEADERS[14]]]);
    return;
  }
