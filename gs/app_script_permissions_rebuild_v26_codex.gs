...
const SPECIAL_LEAVE_TYPE_HEADERS = ["typeKey", "label", "enabled", "sortOrder", "color", "grantHours", "requestMode", "allowHolidayRequest", "dayCountMode"];
    ...
    try {
      ...
      if (action === "save_special_leave_types") {
        ...
        if (sanitizedTypes.length) {
          ...
          resetSpecialLeaveTypesToDefaultSeed(typeSheet);
        }
        recordLog(ss, actorId, actorName, "SpecialLeaveTypeSave", resolveClientIp(json, e), String(sanitizedTypes.length));
        return ContentService.createTextOutput(JSON.stringify({
          result: "success",
          types: sanitizedTypes.length ? sanitizedTypes : readSpecialLeaveTypes(typeSheet)
        })).setMimeType(ContentService.MimeType.JSON);
      }
...
function ensureSpecialLeaveTypeHeaders(sheet) {
  ...
  const headersV2 = ["typeKey", "label", "enabled", "sortOrder", "color", "grantHours", "requestMode"];
  ...
  if (matchesNew) return;
  if (matchesOld) {
    sheet.getRange(1, 1, 1, SPECIAL_LEAVE_TYPE_HEADERS.length).setValues([[...oldHeaders, "grantHours", "requestMode", "allowHolidayRequest", "dayCountMode"]]);
    return;
  }
  if (matchesV2) {
    sheet.getRange(1, 1, 1, SPECIAL_LEAVE_TYPE_HEADERS.length).setValues([[...headersV2, "allowHolidayRequest", "dayCountMode"]]);
    return;
  }
...
function getSeedSpecialLeaveTypes() {
  return [
    { typeKey: "celebration_5", label: "결혼(본인)", enabled: true, sortOrder: 10, color: "emerald", grantHours: 40, requestMode: "day_only", allowHolidayRequest: true, dayCountMode: "calendar_days" },
    { typeKey: "celebration_2", label: "결혼(자녀/형제자매)", enabled: true, sortOrder: 20, color: "emerald", grantHours: 16, requestMode: "day_only", allowHolidayRequest: true, dayCountMode: "calendar_days" },
    { typeKey: "celebration_1", label: "고희(부모)", enabled: true, sortOrder: 30, color: "emerald", grantHours: 8, requestMode: "day_only", allowHolidayRequest: true, dayCountMode: "calendar_days" },
    { typeKey: "celebration_20", label: "출산", enabled: true, sortOrder: 40, color: "rose", grantHours: 160, requestMode: "same_as_annual", allowHolidayRequest: true, dayCountMode: "calendar_days" },
    { typeKey: "condolence_5", label: "조의(배우자/부모/자녀 및 자녀의 배우자)", enabled: true, sortOrder: 50, color: "slate", grantHours: 40, requestMode: "day_only", allowHolidayRequest: true, dayCountMode: "calendar_days" },
    { typeKey: "condolence_4", label: "조의(형제자매)", enabled: true, sortOrder: 60, color: "slate", grantHours: 32, requestMode: "day_only", allowHolidayRequest: true, dayCountMode: "calendar_days" },
    { typeKey: "condolence_3", label: "조의(조/외조부모)", enabled: true, sortOrder: 70, color: "slate", grantHours: 24, requestMode: "day_only", allowHolidayRequest: true, dayCountMode: "calendar_days" },
    { typeKey: "condolence_1", label: "조의(백숙부모)", enabled: true, sortOrder: 80, color: "slate", grantHours: 8, requestMode: "day_only", allowHolidayRequest: true, dayCountMode: "calendar_days" }
  ];
}
...
function resetSpecialLeaveTypesToDefaultSeed(sheet) {
  const defaults = getSeedSpecialLeaveTypes().map((item) => [
    item.typeKey,
    item.label,
    item.enabled,
...
function normalizeSpecialLeaveDayCountMode(value) {
  const safe = String(value || "").trim().toLowerCase();
...
function sanitizeSpecialLeaveTypeObject(raw, fallback) {
  ...
  return {
    ...
    allowHolidayRequest: toBool(raw && raw.allowHolidayRequest, toBool(base.allowHolidayRequest, false)),
    dayCountMode: normalizeSpecialLeaveDayCountMode((raw && raw.dayCountMode) || base.dayCountMode)
  };
}
